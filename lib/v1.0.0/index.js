...
style.addEventListener("load", () => {
  turtley.ready = true;
  if (typeof turtley.onReady === "function")
    turtley.onReady();
}, { once: true });
...
turtley.Turtle = class Turtle {
  ...
  init(options = {}) {
    ...
    this._fullscreen = this._width === null || this._height === null || Boolean(options.fullscreen);
    ...
    if (this._fullscreen) {
      if (appendTo === document.body) document.documentElement.classList.add("turtley-fullscreen");
  ...
  resize() {
    ...
    if (this._fullscreen) {
      if (this._width === null || this._height === null) {
        ...
        if (xGap < yGap) {
          this._visibleCanvas.width = containerWidth;
          this._visibleCanvas.height = (this._height / this._width) * this._visibleCanvas.width;
        } else {
          this._visibleCanvas.height = containerHeight;
          this._visibleCanvas.width = (this._width / this._height) * this._visibleCanvas.height;
        }
      }
    } else {
    ...
    this._hiddenCanvas.width = Math.max(this._hiddenCanvas.width, this._visibleCanvas.width*1);
    this._hiddenCanvas.height = Math.max(this._hiddenCanvas.height, this._visibleCanvas.height*1);
  ...
  _update() {
    ...
    switch (current.type) {
      ...
      case "backgroundColor":
        ...
        break;
      case "fill":
        if (!this._currentPath) return current.error("No shape to fill. Try calling penDown() first.");
      ...
      case "stepBackward":
        ...
        if (this._penDown && !this._hiddenCtx.isPointInPath(this._currentPath, this._x, this._y)) {
        ...
        const moveX = Math.cos(this._rotation) * moveBy * (current.type === "stepBackward" ? -1 : 1);
        const moveY = Math.sin(this._rotation) * moveBy * (current.type === "stepBackward" ? 1 : -1);
      ...
      case "rotateLeft":
      case "rotateRight":
        const moveByRadians = moveBy * Math.PI / 180 * (current.type === "rotateLeft" ? 1 : -1);
  ...
  setSpeed(speed) {
    return new Promise((resolve, reject) => {
      ...
      this._queue.push({ type: "setSpeed", count: 1, speed, whenDone: resolve, error: reject });
    });
  }
  ...
  setPenColor(color) {
    return new Promise((resolve, reject) => {
      ...
      if (!turtley.util.isColor(color)) return current.error(`Invalid pen color: ${JSON.stringify(current.color)}`);
      
      this._queue.push({ type: "penColor", count: 1, color, whenDone: resolve, error: reject });
    });
  }
  ...
  setFillColor(color) {
    return new Promise((resolve, reject) => {
      ...
      if (!turtley.util.isColor(color)) return current.error(`Invalid fill color: ${JSON.stringify(current.color)}`);

      this._queue.push({ type: "fillColor", count: 1, color, whenDone: resolve, error: reject });
    });
  }
  ...
  setBackgroundColor(color) {
    return new Promise((resolve, reject) => {
      ...
      if (!turtley.util.isColor(color)) return current.error(`Invalid background color: ${JSON.stringify(current.color)}`);

      this._queue.push({ type: "backgroundColor", count: 1, color, whenDone: resolve, error: reject });
    });
  }
  ...
  forward(stepCount) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof stepCount !== "number") return reject(`Parameter "stepCount" must be a number`);
      if (stepCount < 1) return reject(`Parameter "stepCount" must be at least 1`);
      
      this._queue.push({ type: "stepForward", count: stepCount, whenDone: resolve, error: reject });
    });
  }
  ...
  backward(stepCount) {
    return new Promise((resolve, reject) => {
      ...
      this._queue.push({ type: "stepBackward", count: stepCount, whenDone: resolve, error: reject });
    });
  }
  ...
  rotateLeft(degrees) {
    return new Promise((resolve, reject) => {
      ...
      this._queue.push({ type: "rotateLeft", count: degrees, whenDone: resolve, error: reject });
    });
  }
  ...
  rotateRight(degrees) {
    return new Promise((resolve, reject) => {
      ...
      this._queue.push({ type: "rotateRight", count: degrees, whenDone: resolve, error: reject });
    });
  }
